...
allowPort() {
	...
	if systemctl status netfilter-persistent 2>/dev/null | grep -q "active (exited)"; then
		...
		if ! iptables -L | grep -q "$1(xiaojiapp)"; then
			updateFirewalldStatus=true
			iptables -I INPUT -p tcp --dport "$1" -m comment --comment "allow $1(xiaojiapp)" -j ACCEPT
		fi
...
nginxBlog() {
	...
	if [[ -d "/usr/share/nginx/html" && -f "/usr/share/nginx/html/check" ]]; then
		...
		if [[ "${nginxBlogInstallStatus}" == "y" ]]; then
			...
			wget -q -P /usr/share/nginx https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/fodder/blog/unable/html${randomNum}.zip >/dev/null
...
customCDNIP() {
	...
	echoContent yellow "# 注意事项"
	echoContent yellow "\n教程地址:"
	echoContent skyBlue "https://github.com/xiaojiapp/v2ray-agent/blob/master/documents/optimize_V2Ray.md"
...
updateNginxBlog() {
	...
	echoContent yellow "1.登录界面1"
	echoContent yellow "2.登录界面2"
	echoContent yellow "3.登录界面3"
	echoContent yellow "4.SoundWave"
	echoContent yellow "5.Halloween"
	...
	if [[ "${selectInstallNginxBlogType}" =~ ^[1-9]$ ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /usr/share/nginx "https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		else
			wget -c -P /usr/share/nginx "https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/fodder/blog/unable/html${selectInstallNginxBlogType}.zip" >/dev/null
		fi
...
updateV2RayAgent() {
	...
	if wget --help | grep -q show-progress; then
		wget -c -q --show-progress -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/install.sh"
	else
		wget -c -q -P /etc/v2ray-agent/ -N --no-check-certificate "https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/install.sh"
	fi
	...
	echoContent skyBlue "wget -P /root -N --no-check-certificate https://raw.githubusercontent.com/xiaojiapp/v2ray-agent/master/install.sh && chmod 700 /root/install.sh && /root/install.sh"
	echo
	exit 0
}
...
aliasInstall() {
	...
	if [[ -f "$HOME/install.sh" ]] && [[ -d "/etc/v2ray-agent" ]] && grep <"$HOME/install.sh" -q "作者:xiaojiapp"; then
...
dokodemoDoorUnblockStreamingMedia() {
	...
	echoContent red "\n=============================================================="
	echoContent yellow "# 注意事项"
	echoContent yellow "任意门解锁详解，请查看此文章[https://github.com/xiaojiapp/v2ray-agent/blob/master/documents/netflix/dokodemo-unblock_netflix.md]\n"
